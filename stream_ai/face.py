...
#     s.connect(ADDR)
#     s.send('[Multibots:PASSWD]'.encode())
#     time.sleep(0.5)
...
#     s.connect((HOST, PORT)) 
#     def sendingMsg(): 
#         s.send('[Turtlebot:PASSWD]'.encode()) 
#         time.sleep(0.5)
#         while True: 
#             data = input() 
    ...
    def draw_detections_function(frame):
        ...
        for roi, landmarks, identity in zip(*detections):
            ...
            if identity.id != FaceIdentifier.UNKNOWN_ID:
                #percentage = (100.0 * (1 - identity.distance))
                text += " %.2f%%" % (100.0 * (1 - identity.distance))
                
                # if percentage >= 80.0:
                #     send_message_to_server("{percentage)}") 
    ...
    def save_face_function():
        for idx, (roi, _, identity) in enumerate(zip(*detections)):
            if identity.id == FaceIdentifier.UNKNOWN_ID:
                ...
                file_pattern = re.compile(r"target.jpg")
                existing_numbers = []
                ...
                new_filename = f"{face_dir}target.jpg"
...
if __name__ == "__main__":
    ...
    try:
        ...
        pass
    # Use a webcam stream from Jetson Nano
    url = "http://10.10.14.168:8080/?action=stream"
    ...
    while True:
        ...
        resized_frame = cv2.resize(frame, (544, 320))
        ...
        with lock:
                ...
                cv2.imshow("Front Camera", images)
        ...
        if key & 0xFF == ord("q") or key == 27:
            ...
            save_face(detections, resized_frame)
        elif key == ord("r"):
            reload()
