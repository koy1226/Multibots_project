            ...
            with lock:
                ...
                if frame_num == 0:
                    ...
                    if args.output_resolution:
                        output_resolution = output_transform_1.new_resolution
                        output_resolution = output_transform_2.new_resolution

                h, w = frame_1.shape[:2]
                input_image_1 = preprocess(frame_1, detector.height, detector.width)

                output_1 = detector.predict(input_image_1)

                _, f_width = frame_1.shape[:2]
                bbox_xywh, score, label = process_results(h, w, results=output_1)

                img_crops = []
                for box in bbox_xywh:
                    x1, y1, x2, y2 = xywh_to_xyxy(box, h, w)
                    img = frame_1[y1:y2, x1:x2]
                    img_crops.append(img)

                if img_crops:
                    # preprocess
                    img_batch = batch_preprocess(
                        img_crops, extractor.height, extractor.width
                    )
                ...
                if len(outputs) > 0:
                    ...
                    frame_1 = draw_boxes(frame_1, bbox_xyxy, identities)

                detections_2 = frame_processor.process(frame_2)

                draw_detections(
                    frame_2, frame_processor, detections_2, output_transform_2
                )
...
def boundingbox(color_frame, depth_frame, frame, detections):
    def boundingbox_function():
    ...
    boundingbox_thread = threading.Thread(target=boundingbox_function)
    boundingbox_thread.start()
    boundingbox_thread.join()
